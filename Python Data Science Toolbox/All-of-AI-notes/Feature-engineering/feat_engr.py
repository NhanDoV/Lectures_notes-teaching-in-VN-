...
def datetime_enhance(data, date_col='DOB', apply_on=['get_month', 'get_month_name', 'hour']):
    """
        Trích xuất các giá trị liên quan đến thời gian từ một cột date-time (date_col) cho trước    
        For example
        ...
        df.head()
                ...
                +-------------------+------------+---------+--------------+----+----+----+----+----+----+----+                 
    """
    # Decide columns-order
    final_col = [date_col] + list(set(data.columns) - set([date_col])) + apply_on
    
    # Processing
    data[date_col] = pd.to_datetime(data[date_col])
    data['get_date'] = data[date_col].dt.date
    data['get_times'] = data[date_col].dt.timetz
    data['get_month'] = data[date_col].dt.month
    data['get_month_name'] = data[date_col].dt.month_name()
    data['hour'] = data[date_col].dt.hour
    data['day.month'] = data[date_col].dt.day 
    data['week-year'] = data[date_col].dt.isocalendar().week
    data['minute'] = data[date_col].dt.minute
    data['week.month'] = data[date_col].dt.day // 7

    return data[final_col]
...
def create_tsne_dataset(data, col_to_drop, target_col, n_component):
    """
        Parameters
            - data (dataframe)
            - col_to_drop (list of strings) :
    ...
    X = data.drop(columns = col_to_drop + [target_col])
    y = data[target_col]
    tsne = TSNE(n_components=n_component, learning_rate='auto', init='random', 
                perplexity=3, n_iter=500, n_iter_without_progress=100,
                metric='euclidean', method='exact'
                )
    tsne_df = pd.DataFrame(tsne.fit_transform(X.to_numpy()), 
                           columns=[f'tsne_{k+1}' for k in range(n_component)])
